...
jest.mock("@mui/icons-material", () => ({
  ...
  StarOutline: (props: any) => (
    <span onClick={props.onClick} data-testid="StarOutlineIcon">
      ☆
    </span>
  ),
}));
...
describe("RecipeRating", () => {
  ...
  it("renders correct number of stars based on average rating", () => {
    ...
    expect(screen.getAllByTestId("StarIcon")).toHaveLength(3); // full stars
    expect(screen.getAllByTestId("StarHalfIcon")).toHaveLength(1); // half star
    expect(screen.getAllByTestId("StarOutlineIcon")).toHaveLength(1); // empty star
  ...
  it("updates rating after user clicks a star", () => {
    ...
    const outlineStars = screen.getAllByTestId("StarOutlineIcon");
